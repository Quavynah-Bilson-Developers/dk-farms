 ...
 *
 * Glide library module for loading and caching images
 */

@GlideModule
class FarmGlideUtil : AppGlideModule() {
	...
	override fun applyOptions(context: Context, builder: GlideBuilder) {
		...
		//Get prefs
		val api: FarmBaseApi = FarmBaseApi[context]
		
		when {
			activityManager.isLowRamDevice || api.isSavingData -> {
				defaultOptions.format(PREFER_RGB_565)
			}
			else -> defaultOptions.format(PREFER_ARGB_8888)
		}
